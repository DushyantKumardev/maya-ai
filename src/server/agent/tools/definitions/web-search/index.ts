...
interface WebSearchSystemOptions {
...
interface SerperRelatedSearchItem {
  query: string;
}
...
interface SerperExtendedResponse {
  ...
  images?: SerperResultItem[];
  videos?: SerperResultItem[];
  places?: SerperResultItem[];
  scholar?: SerperResultItem[];
  shopping?: SerperResultItem[];
...
export const webSearchTool = {
  ...
  name: "web-search",
  description:
    "Comprehensive research engine. Use the optimal type: places for local queries, images for visual topics, shopping for product discovery, videos for tutorials, scholar for academic research, and search for general snippets. Use queries (max 3) for parallel multi-intent research.",
  parameters: {
    type: "object",
    properties: {
      ...
      queries: {
        type: "array",
        items: { type: "string" },
        description: "List of related sub-topics for parallel research.",
      },
      max: {
        type: "number",
        description: "Max results per query (1-15, default 5).",
      },
      location: {
        type: "string",
        description:
          "Specific location for local search (for example, 'Delhi, India').",
      },
      gl: {
        type: "string",
        description:
          "Two-letter country code for regional search. Defaults to user's current country.",
      },
      type: {
        type: "string",
        enum: [
        ...
        description: "Specialized mode. Default is 'search'.",
      },
      tbs: {
        type: "string",
        description: "Time filter such as qdr:d for 24h or qdr:w for week.",
      },
    },
  },
  execute: webSearch,
...
export async function webSearch(
  args: WebSearchArgs,
  sysOptions: WebSearchSystemOptions = {},
  ...
  const activeGl = (gl || sysLocation?.countryCode || "us")
    .toLowerCase()
    .slice(0, 2);
  const searchQueries = Array.isArray(queries) ? queries : query ? [query] : [];
  const validQueries = searchQueries
  ...
  if (!apiKey) {
    const message =
      "Serper API key is not configured. Please add it in Settings > Connections.";
  ...
  onStatusUpdate?.({
    message: "Searching...",
    data: { isLive: true, activeQueries: validQueries, searchType: type },
  });

  try {
    const taskResponses = await Promise.all(
      validQueries.map((searchQuery) =>
        fetchWithRetry(
          searchQuery,
          clampedMax,
          type,
    ...
    taskResponses.forEach((response, index) => {
      ...
      if (response.peopleAlsoAsk)
        allPeopleAlsoAsk.push(...response.peopleAlsoAsk);
      if (response.relatedSearches)
        allRelated.push(...response.relatedSearches);
    ...
    const peopleAlsoAsk = allPeopleAlsoAsk
      .filter((item) => {
    ...
    onStatusUpdate?.({
      message: `Research engine failed: ${message}`,
      done: true,
    });
    return { results: [], error: message };
  }
...
async function fetchSerper(
  ...
  try {
    ...
    return {
      ...
      peopleAlsoAsk: body.peopleAlsoAsk?.map((item) => ({
...
function parseSerperResponse(
  body: SerperExtendedResponse,
  max: number,
  type: SearchType,
): SearchResult[] {
  ...
  const answerBox = body.answerBox
    ? {
        title: body.answerBox.title,
        answer: body.answerBox.answer || body.answerBox.snippet,
      }
    : undefined;
  const knowledgeGraph = body.knowledgeGraph
    ? {
  ...
  for (const item of rawList) {
    ...
    const result: SearchResult = {
      ...
      url: item.link?.trim() || item.imageUrl || "",
      snippet: item.snippet?.trim() || item.description || "",
    ...
    if (type === "places") {
      ...
      if (!result.imageUrl && item.thumbnailUrl)
        result.imageUrl = item.thumbnailUrl;
    }
    if (type === "scholar")
      result.scholar = { publication: item.publication, citedBy: item.citedBy };
    if (type === "shopping")
      result.shopping = {
        price: item.price,
        source: item.source,
    ...
    if (results.length === 0 && type === "search") {
      if (answerBox) result.answerBox = answerBox;
      if (knowledgeGraph) result.knowledgeGraph = knowledgeGraph;
    }
...
async function fetchWithRetry(
  ...
  for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
    try {
      return await fetchSerper(query, max, type, location, gl, tbs, apiKey);
    } catch (error) {
...
function sanitizeForModel<T>(data: T): T {
  if (Array.isArray(data))
    return data.map((item) => sanitizeForModel(item)) as T;
